...
class GeneticTrainer:
    ...
    def mutate(self, individual: Individual) -> Individual:
        ...
        if self.config.adaptive_mutation and self.best_individual:
            ...
            fitness_ratio = individual.fitness / (self.best_individual.fitness + 1e-8)
            mutation_rate = self.config.mutation_rate_max - (
                (self.config.mutation_rate_max - self.config.mutation_rate_min) * fitness_ratio
            )
        ...
        noise = np.random.normal(0, self.config.mutation_strength, individual.weights.shape)
